...
public class Util {
    ...
    public static HashMap<String, Integer> calcularFrecuencias(String texto) {
        HashMap<String, Integer> hashMapFrecuencia = new HashMap<>();
        for (int i = 0; i < texto.length(); i++) {
            String s = String.valueOf(texto.charAt(i));
            if (!hashMapFrecuencia.containsKey(s)) {
                hashMapFrecuencia.put(s, 1);
            } else {
                hashMapFrecuencia.put(s, hashMapFrecuencia.get(s) + 1);
            }
        }
        return hashMapFrecuencia;
    }
